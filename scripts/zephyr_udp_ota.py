#!/usr/bin/env python3
"""Orchestrate Zephyr OTA over UDP using smpclient (BL-035)."""
...
async def async_main(args):
    ...
    print(f"Connecting to {args.ip}:{args.port} via SMP UDP...")
    transport = SMPUDPTransport(mtu=1500)
    client = SMPClient(transport, args.ip)
    ...
    try:
        ...
        # If slot 1 has an image, erase it upfront
        if any(getattr(img, "slot", None) == 1 for img in img_state.images):
            ...
            try:
                ...
                print(f"Note: Erase exception ({e}). Waiting 5s...")
        ...
        print("[SUCCESS] UDP OTA workflow completed.")
        return 0

    except Exception as e:
        print(f"ERROR during UDP OTA: {e}")
        import traceback
...
def main():
    parser = argparse.ArgumentParser(description="Zephyr UDP OTA Update (BL-035)")
    parser.add_argument("binary", help="Path to signed binary (e.g. zephyr_v1.bin)")
    parser.add_argument(
        "--ip", default=DEFAULT_IP, help=f"Target IP address (default: {DEFAULT_IP})"
    )
    parser.add_argument(
        "--port",
        type=int,
    ...
    parser.add_argument(
        "--confirm", action="store_true", default=True, help="Confirm image after boot"
    )
    parser.add_argument(
        "--no-confirm",
        dest="confirm",
        action="store_false",
        help="Test boot without confirmation",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        dest="reset",
        action="store_false",
        help="Do not reset after upload",
    )
    parser.add_argument(
