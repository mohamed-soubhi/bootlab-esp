...
**Acceptance**
- [x] Every command works on both boards (IDF track verified end-to-end; Zephyr track gated behind BL-063b).
- [x] Mocked unit tests ≥ 80 % line coverage; ruff + mypy clean (BL-046: 115 tests, 84% line coverage, ruff clean, mypy 0 errors).
...
| R14 | ESP32-S3 native USB-Serial-JTAG resets into bootloader on connection, not just intentional reset (transport-level, distinct from R13's descriptor issue) | Sustained bidirectional serial communication with running app firmware (LABID, BL-022) may not be possible from this WSL2/usbipd host at all, not just inconvenienced. Also produced a FALSE SIGNAL during BL-020 bring-up: a single WS2812 frame latched from one loop iteration before the chip reset into bootloader was misread as "wrong GPIO pin, steady non-blinking light," causing an unnecessary GPIO38 detour before GPIO48 was re-confirmed correct. | Confirmed 2026-09-20 on lab-esp-idf: every attempt to open a passive read connection (idf.py monitor with --no-reset, raw `cat`, pyserial with dsrdtr=False/rtscts=False and explicit dtr=False/rts=False) resulted in the chip resetting into ROM download mode (`rst:0x15 USB_UART_CHIP_RESET`) instead of staying in normal app run -- confirmed NOT fixable via userspace DTR/RTS suppression, root cause not fully isolated to a single mechanism (native USB-Serial-JTAG auto-reset hardware vs. the usbipd/WSL2 bridge itself). A true physical USB power-cycle (unplug/replug, bypassing usbipd/DTR-RTS entirely) reliably produced a stable running state: lab-esp-idf blinked white at 1Hz on GPIO48 as designed, confirmed by direct visual count -- proving the app code and GPIO48 were correct all along, and every "steady light" observation before that was this reset behavior, not a code or pin bug. This is a HARD BLOCKER for BL-022 (LABID on IDF needs sustained bidirectional serial) until either (a) a native Linux/Windows-direct test (no usbipd bridge) isolates the actual cause, or (b) a different LABID verification strategy is found that doesn't require a persistent open serial connection during development iteration. Any tool that opens this board's port (idf_monitor, cat, pyserial) risks resetting it mid-operation; only a true physical power-cycle reliably yields a stable observation window. **UPDATE 2026-09-21 (root cause narrowed):** with the idf board detached from usbipd and opened natively on Windows (COM14, `python -m serial.tools.miniterm --dtr 0 --rts 0 COM14 115200`), the port opened with NO reset: `blink_diag` logs streamed continuously at 500 ms on / 500 ms off (1 Hz) and the LED kept blinking. So the reset-on-open is caused by the usbipd/WSL2 bridge, not the chip's USB-Serial-JTAG hardware. Fresh-boot case also confirmed 2026-09-21: after a real unplug/replug the tick counter restarted (first log line at ~2.6 s) and the native port open (DTR/RTS forced inactive) again caused no reset and no blink interruption. R14 is WSL2/usbipd-only. Implication: BL-022 is not blocked on the real RPi4 rig (no usbipd); on WSL2 dev use native Windows serial, or the RPi4. **UPDATE 2026-09-21 (switching from WSL2):** `usbipd` attach and detach can be driven directly from the WSL2 bash shell without leaving the terminal: `powershell.exe -Command "usbipd attach --wsl --busid 7-4" < /dev/null` and `powershell.exe -Command "usbipd detach --busid 7-4" < /dev/null`. The `< /dev/null` stdin redirection is mandatory to prevent `powershell.exe` from hanging indefinitely waiting for stdin in WSL2 subshells. |
| R15 | IDF per-variant `-B build_*` dirs share ONE project-level `esp_idf/sdkconfig` (gitignored) | Wrong variant silently built and flashed: on 2026-09-21 `idf.py -B build_hang ...` wrote `CONFIG_APP_VARIANT_HANG=y` into the shared sdkconfig, so the next plain `idf.py flash` of `build/` regenerated and compiled the HANG variant (new ELF SHA + compile time, so it looked like a fresh v1 build). Observed as a board in a 5 s task-WDT reboot loop after 'flashing v1'. Also, a `build*/config/sdkconfig.h` read BEFORE `idf.py` re-runs cmake can be stale, so it is not proof of what gets compiled. | Give every build dir its own sdkconfig: `idf.py -B <dir> -DSDKCONFIG=<dir>/sdkconfig -DSDKCONFIG_DEFAULTS="sdkconfig.defaults[;variant.defaults]" build` (and delete any stale `esp_idf/sdkconfig`). Before ANY flash, verify the variant AFTER the build from `<dir>/sdkconfig`/`config/sdkconfig.h`, then confirm on-target: the boot log's `App version`/`Compile time`/ELF SHA must match the build just made and the behavior must match the variant. Fixed for v1 on 2026-09-21 (`build/sdkconfig`, `CONFIG_APP_VARIANT_V1`). The 2026-09-20 variant builds (v2/no_confirm/bad_sig) predate this finding: their headers were checked right after each build, but rebuild them with per-dir sdkconfig before relying on them for BL-021/BL-031 flashes. |
