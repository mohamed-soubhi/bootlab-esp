...
| R14 | ESP32-S3 native USB-Serial-JTAG resets into bootloader on connection, not just intentional reset (transport-level, distinct from R13's descriptor issue) | Sustained bidirectional serial communication with running app firmware (LABID, BL-022) may not be possible from this WSL2/usbipd host at all, not just inconvenienced -- this is a standing risk to BL-022, not a solved problem | Confirmed 2026-09-20 on lab-esp-idf: every attempt to open a passive read connection (idf.py monitor with --no-reset, raw `cat`, pyserial with dsrdtr=False/rtscts=False and explicit dtr=False/rts=False) resulted in the chip resetting into ROM download mode (`rst:0x15 USB_UART_CHIP_RESET`) instead of staying in normal app run. This survived explicit DTR/RTS suppression at the pyserial level, ruling out simple tty-default-DTR-on-open as the sole cause -- points at either the chip's native USB-Serial-JTAG auto-reset hardware reacting to any USB control-line-state change (not just classic DTR/RTS bit-banging), or the usbipd/WSL2 bridge altering control-line behavior in a way userspace tools on the Linux side can't override. Not yet root-caused or fixed. BL-022 (LABID on IDF, requires ID?/VER?/STATE? bidirectional serial traffic) should treat this as an open blocker to investigate directly (e.g. testing on native Linux/Windows without usbipd) before assuming LABID integration will work on this host, not just build around it. |
