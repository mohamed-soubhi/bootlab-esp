...
@pytest.mark.zephyr
@pytest.mark.labid
def test_t01_factory_v1_boot_zephyr(hil_rig: HilRig) -> None:
    """T01 (zephyr): Factory v1 boots, measure 1 Hz, LABID info valid."""
    status = hil_rig.state()
    if status:
        assert status.get("app") == "1.0.0", f"Expected app 1.0.0 on factory v1 boot, got {status.get('app')}"
        assert status.get("confirmed") is True, "Expected confirmed=True on factory v1"
    ...
    report = (
        f"T01 Factory Boot Report (zephyr)\n"
        f"Board: {hil_rig.board}\n"
        f"App Version: 1.0.0\n"
        f"Slot: {(status or {}).get('slot')}\n"
        f"Confirmed: True\n"
    ...
    hil_rig.log_artifact("t01_factory_boot_zephyr.txt", report)


@pytest.mark.zephyr
@pytest.mark.ble
def test_t02_update_v1_to_v2_ble_zephyr(hil_rig: HilRig) -> None:
    """T02 (zephyr): Update v1 -> v2 over BLE SMP, 4 Hz, confirmed=1."""
    ok = hil_rig.update_ota(variant="v2", transport="ble")
    assert ok, "BLE SMP OTA update v1 -> v2 failed"

    status = hil_rig.state()
    if status:
        assert status.get("app") == "2.0.0", f"Expected app 2.0.0, got {status.get('app')}"
        assert status.get("confirmed") is True, "Expected confirmed=True for v2"
    ...
    hil_rig.log_artifact("t02_ble_update_zephyr.txt",
                          f"T02 BLE Update OK (zephyr): 2.0.0, slot {(status or {}).get('slot')}, {measured_hz:.2f} Hz\n")


@pytest.mark.zephyr
@pytest.mark.udp
def test_t02_update_v1_to_v2_udp_zephyr(hil_rig: HilRig) -> None:
    """T02 (zephyr): Update v1 -> v2 over UDP SMP, 4 Hz, confirmed=1."""
    ok = hil_rig.update_ota(variant="v2", transport="udp")
    assert ok, "UDP SMP OTA update v1 -> v2 failed"
    ...
    hil_rig.log_artifact("t02_udp_update_zephyr.txt",
                          f"T02 UDP Update OK (zephyr): 2.0.0, slot {(status or {}).get('slot')}, {measured_hz:.2f} Hz\n")


@pytest.mark.zephyr
@pytest.mark.ble
def test_t03_downgrade_v2_to_v1_ble_zephyr(hil_rig: HilRig) -> None:
    """T03 (zephyr): Downgrade v2 -> v1 over BLE SMP, 1 Hz, confirmed=1."""
    assert hil_rig.ensure_variant("v2", "udp"), "precondition failed: board could not be put on v2"
    ok = hil_rig.update_ota(variant="v1", transport="ble")
    assert ok, "BLE SMP OTA downgrade v2 -> v1 failed"

    status = hil_rig.state()
    if status:
        assert status.get("app") == "1.0.0", f"Expected app 1.0.0, got {status.get('app')}"
        assert status.get("confirmed") is True, "Expected confirmed=True for v1"
    ...
    hil_rig.log_artifact("t03_ble_downgrade_zephyr.txt",
                          f"T03 BLE Downgrade OK (zephyr): 1.0.0, slot {(status or {}).get('slot')}, {measured_hz:.2f} Hz\n")


@pytest.mark.zephyr
@pytest.mark.udp
def test_t03_downgrade_v2_to_v1_udp_zephyr(hil_rig: HilRig) -> None:
    """T03 (zephyr): Downgrade v2 -> v1 over UDP SMP, 1 Hz, confirmed=1."""
    assert hil_rig.ensure_variant("v2", "udp"), "precondition failed: board could not be put on v2"
    ok = hil_rig.update_ota(variant="v1", transport="udp")
    assert ok, "UDP SMP OTA downgrade v2 -> v1 failed"
    ...
    hil_rig.log_artifact("t03_udp_downgrade_zephyr.txt",
                          f"T03 UDP Downgrade OK (zephyr): 1.0.0, slot {(status or {}).get('slot')}, {measured_hz:.2f} Hz\n")
